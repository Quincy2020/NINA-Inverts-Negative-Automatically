...
from qnegative.ui.roll_selection_controller import RollSelectionController
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.export_controller = ExportController(
            ...
            batch_dialog=self.batch_export_dialog,
        )
        self.batch_export_dialog.pauseRequested.connect(self.export_controller.pause_batch_export)
        self.batch_export_dialog.resumeRequested.connect(self.export_controller.resume_batch_export)
        self.batch_export_dialog.cancelRequested.connect(self.export_controller.cancel_batch_export)
        ...
        self.roll_selection = RollSelectionController(self.filmstrip)
        self.filmstrip.hide()
    ...
    def white_balance_point_selected(self, point: ImagePoint) -> None:
        ...
        try:
            ...
            pick_result = self.white_balance.pick_printer_balance(
                point=point,
                log_point=log_point,
        ...
        updated = deepcopy(self.adjustments)
        updated.printer_balance = pick_result.printer_balance
        self.control_panel.set_adjustments(updated, emit=True)
        self.preview_view.set_tool_mode(ToolMode.PAN)
    ...
    def adjustments_changed(self, values: dict) -> None:
        ...
        wb_controls_changed = printer_balance_changed or auto_cmy_strength_changed
        if previous.auto_wb != self.adjustments.auto_wb:
            self._clear_lab_print_cmy_offsets()
            if self.current_path is not None:
                self.preview_result_cache.pop(self.current_path, None)
            self._reset_preview_stage_caches()
        elif wb_controls_changed:
            if not self.white_balance.retarget_cmy_offsets(previous, self.adjustments):
                self._clear_lab_print_cmy_offsets()
            if self.current_path is not None:
                self.preview_result_cache.pop(self.current_path, None)
            self._reset_preview_stage_caches()
    ...
    def _preview_render_finished(self, job_id: int, output: PreviewRenderOutput, show_errors: bool) -> None:
        ...
        if output.quality == FINAL_RENDER_QUALITY:
            self.white_balance.set_cmy_offsets(output.lab_print_cmy_offsets, self.adjustments)
    ...
    def _current_lab_print_log_floors(self) -> list[float] | None:
        if self._last_untransformed_negative_result is not None:
            return _array_to_float_list(self._last_untransformed_negative_result.lab_print_log_floors)
        if self.current_path is None:
            return None
        state = self.image_states.get(self.current_path)
        return deepcopy(state.lab_print_log_floors) if state and state.lab_print_log_floors else None

    def _current_lab_print_log_ceils(self) -> list[float] | None:
        if self._last_untransformed_negative_result is not None:
            return _array_to_float_list(self._last_untransformed_negative_result.lab_print_log_ceils)
        if self.current_path is None:
            return None
        state = self.image_states.get(self.current_path)
        return deepcopy(state.lab_print_log_ceils) if state and state.lab_print_log_ceils else None
    ...
    def _clear_lab_print_cmy_offsets(self) -> None:
        self.white_balance.clear_cmy_offsets()
    ...
    def export_current(self) -> None:
        self.export_controller.export_current()

    def export_completed(self) -> None:
        self.export_controller.export_completed()

    def export_selected(self, paths=None) -> None:
        self.export_controller.export_selected(paths)

    def remove_from_roll(self, paths) -> None:
        remove_paths = self.export_controller.selected_export_paths(paths)
        if not remove_paths:
            return
        names = ", ".join(path.name for path in remove_paths[:3])
        if len(remove_paths) > 3:
            names += f", and {len(remove_paths) - 3} more"
        answer = QMessageBox.question(
            self,
            "Remove From Roll",
            f"Remove {len(remove_paths)} image(s) from this roll?\n\n{names}\n\nThe source files will not be deleted.",
        )
        if answer != QMessageBox.Yes:
            return

        current_removed = self.current_path in set(remove_paths)
        next_path = self._next_path_after_removal(remove_paths) if current_removed else self.current_path
        if self.current_path is not None:
            self._save_current_state()
        self._project.exclude_from_roll(remove_paths)
        self.folder_files = list(self._project.folder_files)
        self.roll_selection.clear_selection()
        self.filmstrip.set_files(self.folder_files, next_path)
        self.roll_selection.set_files(self.folder_files, next_path)
        self._restore_filmstrip_session_badges()
        self._schedule_roll_session_save()

        if next_path is not None and next_path in self.folder_files:
            self.load_path(next_path, refresh_sequence=False)
        elif self.folder_files:
            self.load_path(self.folder_files[0], refresh_sequence=False)
        else:
    ...
    def _next_path_after_removal(self, remove_paths: list[Path]) -> Path | None:
        if self.current_path is None:
            return None
        remove_set = set(remove_paths)
        remaining = [path for path in self.folder_files if path not in remove_set]
        if not remaining:
            return None
        try:
            index = self.folder_files.index(self.current_path)
        except ValueError:
            return remaining[0]
        for candidate in self.folder_files[index + 1 :]:
            if candidate not in remove_set:
                return candidate
        for candidate in reversed(self.folder_files[:index]):
            if candidate not in remove_set:
                return candidate
        return remaining[0]
    ...
    def _save_current_state(self) -> None:
        ...
        self.image_states[self.current_path] = build_current_image_state(
            ...
            lab_print_log_floors=(
                _array_to_float_list(self._last_untransformed_negative_result.lab_print_log_floors)
                if self._last_untransformed_negative_result is not None
                else (
                    self.image_states[self.current_path].lab_print_log_floors
                    if self.current_path in self.image_states
                    else None
                )
            ),
            lab_print_log_ceils=(
                _array_to_float_list(self._last_untransformed_negative_result.lab_print_log_ceils)
                if self._last_untransformed_negative_result is not None
                else (
                    self.image_states[self.current_path].lab_print_log_ceils
                    if self.current_path in self.image_states
                    else None
                )
            ),
    ...
    def _restore_state_for_path(self, path: Path) -> bool:
        ...
        self.white_balance.restore(
            point=restored.white_balance_point,
            cmy_offsets=restored.lab_print_cmy_offsets,
            cmy_strength=restored.lab_print_cmy_strength,
        )
    ...
    def _start_next_preinvert_jobs(self) -> None:
        if self.export_controller.export_in_progress:
            self._refresh_activity_progress()
            return
        jobs = self._frame_automation.start_next_preinvert_jobs(
            can_start_path=self._should_preinvert_path,
            export_in_progress=self.export_controller.export_in_progress,
        )
        for job in jobs:
            task = PreInvertTask(
                job_id=job.job_id,
    ...
    def _set_folder_sequence(self, path: Path) -> None:
        ...
        self.roll_selection.set_files(self.folder_files, path)
        self._restore_filmstrip_session_badges()
    ...
    def _sync_sequence_position(self, path: Path) -> None:
        ...
        self.roll_selection.set_active(path)
...
def _array_to_float_list(values) -> list[float] | None:
    if values is None:
        return None
